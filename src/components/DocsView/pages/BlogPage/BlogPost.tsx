...
export default class BlogPost extends React.Component<{}, {}> {
  render() {
    return (
      <div
        className={cx($p.flex, $p.flexColumn, $p.bgBlack04, $p.contentCenter, $p.itemsCenter, $p.mt38, $p.ph96, $p.flexWrap)}>
        <AnnoucementIcon/>
        <div className={cx($p.flex, $p.mt60, $p.flexWrap)}>
          <div>
            <h1 className={cx($p.f38, $p.fw3, $p.mb16)}>User authentication in GraphQL with Auth0 and Digits</h1>
            <div className={cx($p.flex, $p.black20, $p.itemsCenter, $p.mb38, $p.relative, $p.flexWrap)}>
