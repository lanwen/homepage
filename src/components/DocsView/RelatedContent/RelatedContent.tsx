...
export default class RelatedContent extends React.Component<{}, {}> {
  ...
  render() {
    const Container = styled.div`
      background: linear-gradient(to top, #fff 05%, rgba(0, 0, 0, 0.03) 15%,rgba(0, 0, 0, 0.03) 95%,#fff 85%);
`
...
`
    return (
      <div>
        <div className={cx($p.bBlack50, $p.bt, $p.mv38, $p.pv16, $p.flex, $p.justifyCenter)}>
        ...
        <Container className={cx($p.flex, $p.justifyCenter)}>
          ...
          <Rest className={cx($p.mt96)}>
            <p className={cx($p.fw6, $p.f16, $p.black30, $p.ph25, $p.pb25)}>FURTHER READING</p>
            <div className={cx($p.flex, $p.ph25, $p.pv10)}>
              <Circle
                background=''
                className={cx($p.bbox, $p.db, $p.mr16, $p.mt4)}
